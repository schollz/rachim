...
function Rachim:init()
    ...
    local random_notes = {1, 1, 1, 1, 1, 1, 1, 3, 3, 3, 3, 3, 3, 5, 5, 5, 5, 5, 5, 5, 4, 4, 4, 4, 2, 2, 2, 2, 2, 2, 6,
        ...
        while true do
            ...
            if self.is_playing and self.steps >= params:get(self.id .. "dur") * 10 then
                ...
                if note_index > 0 then
                    ...
                    if self.midi_info ~= nil and self.midi_info.midi_device ~= nil then
                        table.insert(self.notes_played, note)
                        local velocity = util.round(util.linlin(-96, 16, 0, 127, params:get(self.id .. "db")))
                        velocity = util.clamp(velocity, 0, 127)
                        self.midi_info.midi_device:note_on(note, velocity, self.midi_info.midi_channel)
                        self.midi_info.active_notes[note] = true
                    end
                end
                self.steps = 0
            elseif self.is_playing and self.steps >=
                util.round(params:get(self.id .. "dur") * 10 * params:get("fill") / 100) then
                engine.set(self.id, "gate", 0)
                if self.midi_info ~= nil and self.midi_info.midi_device ~= nil then
                    for _, note in ipairs(self.notes_played) do
                        self.midi_info.midi_device:note_off(note, nil, self.midi_info.midi_channel)
                        self.midi_info.active_notes[note] = nil
                    end
                    self.notes_played = {}
                end
            end
